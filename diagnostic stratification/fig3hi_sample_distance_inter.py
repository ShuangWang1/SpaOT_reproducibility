...
adata_bz = sc.read_h5ad('data/BZ.h5ad')
...
def clean_marker(name):
    # remove leading numeric + metal-isotope tokens ending in Di
    name = re.sub(r'^.*?Di\s*', '', name)
    return name.strip().lower()

bz_clean = pd.Index([clean_marker(x) for x in adata_bz.var_names])
adata_clean = pd.Index([x.lower() for x in adata.var_names])

synonyms = {
    # Pan-Cytokeratin/Epithelial Markers
    "pancyto": "panck",
    "cytoker": "panck",
    "keratin": "panck",
    "ecadhe": "e_cadherin",
    ...
    "ki67": "ki67",
    "histone": "hh3_total",        # Mapped to total Histone H3
    "phospho histone": "hh3_ph",   # Mapped to Phospho Histone H3
    
    # Signaling / Kinase Pathway Markers
    "egfr": "egfr",
    "cerbb": "her2",               # Maps ERBB/CERBB to HER2
    "phospho s6": "ps6",
    "phospho mtor": "pmtor",
    ...
    "cleaved": "c_parp_c_casp3",   # Uses the combined marker from adata_clean
    "p53": "p53",
    ...
    # Immune / Hematopoietic Markers
    "cd20": "cd20",
    "cd68": "cd68",
    "cd44": "cd44",
    "cd45": "cd45",
    "cd3epsi": "cd3",              # Assumed match: CD3 epsilon subunit -> CD3
    
    # Vascular / Endothelial Marker
    "vwf": "vwf_cd31",             # Maps VWF to the combined VWF/CD31 feature
    
    # Other Functional Markers
    "carboni": "caix",             # Maps 'carboni' to Carbonic Anhydrase IX (CAIX)

    # Note: 'sma' appears in both, but without context, it's safer to map it explicitly.
    "sma": "sma"
}

bz_mapped = [
    synonyms.get(x, x) for x in bz_clean
]
bz_mapped = pd.Index(bz_mapped)

X_bz = pd.DataFrame(
    adata_bz.X,
    index=adata_bz.obs_names,
    columns=bz_mapped
)
X_bz_collapsed = X_bz.groupby(axis=1, level=0).sum()
common_markers = X_bz_collapsed.columns.intersection(adata_clean)
X_bz_common = X_bz_collapsed[common_markers]

import anndata as ad
adata_bz_aligned = ad.AnnData(
    X=X_bz_common.values,
    obs=adata_bz.obs.copy(),
    uns=adata_bz.uns.copy(),
    obsm=adata_bz.obsm.copy(),
    var=pd.DataFrame(index=common_markers)
)
adata_aligned = adata[:, adata_clean.isin(common_markers)].copy()
adata_aligned.var_names = adata_aligned.var_names.str.lower()
adata_aligned = adata_aligned[:, common_markers].copy()

#normalize and scal
cofactor = 5  # standard for IMC/CyTOF
adata_aligned.X = np.arcsinh(adata_aligned.X / cofactor)
adata_bz_aligned.X = np.arcsinh(adata_bz_aligned.X / cofactor)

X_all = np.vstack([adata_aligned.X, adata_bz_aligned.X])
mean = X_all.mean(axis=0)
std = X_all.std(axis=0) + 1e-8
adata_aligned.X = (adata_aligned.X - mean) / std
adata_bz_aligned.X = (adata_bz_aligned.X - mean) / std

import pickle
#remove the nan value of metabric
with open('img_celldata_metabric.pkl','rb') as f:
    data = pickle.load(f)
data = {int(k): v for k, v in data.items()}

idx_set = list(data.keys())
adata_aligned = adata_aligned[adata_aligned.obs['ImageNumber'].isin(idx_set)].copy()
...
def global_calc_fgw(M,C1,C2,t1masses,t2masses,mass,Lambda,alpha,amijo,verbose,symmetric,measure='fpgw'):
    ...
    if measure =='fmpgw':
        ...
        ot.gromov.fused_unbalanced_gromov_wasserstein(M = M,Cx = C1,Cy = C2,wx=t1masses,wy=t2masses,alpha=alpha,verbose=verbose)
        gamma=result['gamma']
...
class Fused_Partial_Gromov_Wasserstein_distance(Fused_Gromov_Wasserstein_distance):
     ...
     def calc_fgw(self,M,C1,C2,t1masses,t2masses):
        ...
        if self.measure =='fmpgw':
            ...
            result=ot.gromov.fused_unbalanced_gromov_wasserstein(M = M,Cx = C1,Cy = C2,wx=t1masses,wy=t2masses,alpha=self.alpha,verbose=False)
            gamma=result[0]
            cost=fused_pgw_cost(M,C1,C2,gamma,self.alpha,loss_fun='square_loss')
            return gamma,cost
     ...
     def compute_all_distance(self, X, Y, matrix=None): 
        ...
        if matrix is not None:
            ...
            if X.shape[0] == Y.shape[0] and np.all(X == Y):
                ...
                with open(f'BZ_res/results_buffer_{args.start}.pkl', 'wb') as f:
                    pickle.dump(results_buffer, f) 
                
                # with ProcessPoolExecutor(max_workers=num_workers) as executor:
                #     #futures = [executor.submit(compute_batch, batch,self.method,self.features_metric,self.measure,self.mass,self.Lambda,self.alpha,self.amijo,self.verbose,self.symmetric) for p in pairs]
                #     #for future in tqdm(as_completed(futures), total=len(futures), desc="Computing distances"):
                    
                #     futures = {executor.submit(compute_batch, batch,self.method,self.features_metric,self.measure,self.mass,self.Lambda,self.alpha,self.amijo,self.verbose,self.symmetric): batch
                ...
                #                 with open('results_buffer_normalized.pkl', 'wb') as f:
                #                     pickle.dump(results_buffer, f)
                #         except Exception as exc:
                #             print(f"Processing batch generated an exception: {exc}")
                ...
                D = np.zeros((X.shape[0], Y.shape[0]))            
                pairs = [(i, j, X[i], Y[j]) for i in range(len(X)) for j in range(len(Y))]
                ...
                with open(f'inter_res_fugw/results_buffer_{args.start}.pkl', 'wb') as f:
                    pickle.dump(results_buffer, f) 
...
def load_CRC(df):
    ...
    df1 = df[df['groups']==1]
    group1 = df1['patients'].unique()
    df2 = df[df['groups']==2]
    group2 = df2['patients'].unique()
    ...
    for spot, group in df.groupby("spots"):
        ...
        coord = np.array(list(nx.get_node_attributes(G, 'pos').values()))
        g.C = cdist(coord, coord, metric='euclidean')
        g.N = len(G.nodes())
        
        X_sub = np.array(list(nx.get_node_attributes(G, 'expression').values()))
        X_sub_dict = {i:v for i,v in zip(list(G.nodes),X_sub)}
        g.add_attibutes(X_sub_dict)
        
        for i,j in G.edges():
            g.add_edge((i,j))
        
        g.nx_graph.graph['group'] = group['patients'][0]
            
        FGW_graph_list.append(g)
...
def load_meta(adata):
    ...
    FGW_graph_list = []
    for spot, group_adata in adata_dict.items():
        coord = group_adata.obsm["spatial"]     
...
def load_BZ(adata):
    ...
    FGW_graph_list = []
    for spot, group_adata in adata_dict.items():
        coord = group_adata.obsm["spatial"].astype(float)  
...
#load Metabric data
X = load_meta(adata_aligned)
X = np.array(X)

Y = load_BZ(adata_bz_aligned)
Y = np.array(Y)

FPGW.compute_all_distance(X,Y)
