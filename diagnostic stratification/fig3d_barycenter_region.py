...
df = df[df['patients']==args.patient]
...
for spot, group in df.groupby("spots"):
    ...
    if group['Region'].iloc[0] == regs[0]:
        G_list1.append(G)
    elif group['Region'].iloc[0] == regs[1]:
        G_list2.append(G)

import pickle
for region, G_list in zip(regs,[G_list1,G_list2]):
    ...
    with open(f"bary/barycenter_crc_p{args.patient}_{region}.pkl", "wb") as f:
